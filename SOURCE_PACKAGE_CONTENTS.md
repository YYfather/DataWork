...
- 专业模式 MANOVA 因变量无序组合、稳定命名和双入口完整工作流；
- 因变量、因素与拆分统一任务树，200/1000 任务门禁和 Holm 跨模型校正；
- 页面、Excel、Markdown、JSON、ZIP 与 AI 上下文使用一致的组合标识；
- 全部 47 个计算核心完成状态审计与独立 R 对照，用户可见方法不再保留实验性标签；
- V1.5/V1.6 保存计划兼容，MANOVA 失败路径改为明确可定位错误。

## V1.6 发布重点（保留兼容）

- 配对列连续新增、参数继承、“以此新增”和编辑/新增状态隔离；
- 独立公式模板、单位/含义建议、自定义文本与 0–8 位精度选择；
- 配对公式安全白名单 `abs(...)` 与 `|表达式|` 规范化，普通自定义列语义保持不变；
- V1.5 计划兼容、V1.6 后端能力检查、完整预检审核与 R/Python 绝对值对照。
