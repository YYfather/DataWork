...
const state = {
  ...
  pdf: null,
  page: 1,
  mode: localStorage.getItem('reader:mode') || 'page',
  zoom: 1,
  fit: true,
  epoch: 0,
  observer: null,
...
function saveProgress() {
  ...
  localStorage.setItem('reader:last-book', state.currentBook.path);
  updateProgressUI();
}
...
function updateRecent() {
  const lastPath = localStorage.getItem('reader:last-book');
...
$('themeToggle').addEventListener('click', () => {
  ...
  localStorage.setItem('reader:theme', night ? 'night' : 'paper');
...
document.documentElement.dataset.theme = localStorage.getItem('reader:theme') || 'paper';
refreshIcons();
loadLibrary();
