  ...
  for (const entry of await readdir(dir, { withFileTypes: true })) {
    ...
    if (entry.isDirectory()) files.push(...await scan(path, relativePath));
      ...
      files.push({
        ...
        url: `../reader/source/${relativePath.split('/').map(encodeURIComponent).join('/')}`,
...
createServer(async (request, response) => {
  try {
    ...
    if (url.pathname === '/reader-next/api.php') {
      const grouped = {};
    ...
    const requestPath = decoded.startsWith('/reader/source/')
      ? decoded.slice(1)
      : decoded.replace(/^\/reader-next\/?/, '') || 'index.html';
  ...
  console.log(`Reader Next: http://127.0.0.1:${port}/reader-next/`);
});
