# DataWork 1.7.1（V1.7.1）

DataWork 是面向实验数据的跨平台统计分析与可复现项目工作区。统计核心使用 Python、SciPy 和 statsmodels，应用层使用 FastAPI，网页界面使用 Vue 3 + TypeScript。

> 当前源码版本为 `1.7.1`。V1.7.1 修复 Windows CI 的 UTF-8 输出与黄金数据换行稳定性；V1.7 的计算核心审计和 MANOVA 因变量组合功能保持不变。

## 工作树边界

- `main` 工作树是 1.7.1 完整源码，包含 Python 核心、前端、桌面外壳、测试、审计资料和构建器；不保留服务器部署副本或旧 `发布包/`。
- `Web` 工作树是独立的 1.7.0 服务器/网站部署版，只保留生产运行文件、宝塔配置、网站副本和各版本热补丁源码目录；不得反向承载桌面外壳、开发依赖或测试资产。
- 当前尚未发布。热补丁 ZIP 与 `.zip.sha256` 只允许在批准正式发布时由构建器临时生成，不作为任一工作树的常驻产物。
- 服务器升级必须按热补丁目录声明的基线执行；`1.5.0 -> 1.7.0` 补丁不得混用 0.4.9、1.0.0 或未留档的 1.6 基线。

## V1.7 正式版

- 单、双、三及 4–8 因素 MANOVA 均可从候选因变量池生成无序、不重复的组合批次，默认组合大小为 2–2，同一变量可进入多个组合。
- 因变量组合、因素组合与数据拆分统一展开；界面提供任务量上限投影，预检按真实数据给出精确总数，超过 200 强警告、超过 1000 阻止执行。
- 页面、Excel、Markdown、JSON 和 ZIP 统一记录组合名称、成员、大小、因素模型和拆分标签；Excel 提供跨组合聚合表。
- MANOVA 对常数因变量、秩不足、无残差自由度和统计量失败给出明确原因；失败任务保留标识，不再返回含义不明的空结果。
- 全部 47 个可见方法通过 Golden 与 R 4.6.1 独立对照，当前没有仍标记为“实验性”的用户可见计算核心。
- 完整范围与验收证据见 [`DataWork V1.7 更新计划与实施记录`](docs/V1.7_UPDATE_PLAN.md)及[`可复现性记录`](docs/REPRODUCIBILITY.md)。

## V1.6 正式版（保留兼容）

- 配对列名称继续自由输入；添加成功后自动继承名称、来源、公式、单位/含义和小数位，名称获得焦点并全选，也可从任意已有列执行“以此新增”。
- 公式模板与单位/含义建议彼此独立。单位支持常用选项和最长 80 字符自定义文本，小数位使用明确的 0–8 下拉选择。
- 配对公式新增单参数白名单函数 `abs(expression)`；用户输入的非嵌套 `|expression|` 在计划保存时统一规范为 `abs(expression)`。普通自定义列仍只允许基础四则运算。
- “绝对量（保留原始尺度）”只描述数据尺度，不改变符号；数学绝对值只有通过 `abs(...)` 才会实际改变数值。
- 执行审核展示最终列名、来源、规范化公式、单位/含义、精度及无效/缺失数量；旧后端未声明 `pairing_abs` 能力时，包含绝对值公式的计划会在保存或执行前被阻止。
- V1.5 四则运算计划保持兼容。R 4.6.1/Python 对照覆盖 `abs(T)`、`abs(CK)`、`abs(T-CK)` 和相对绝对变化的正数、负数、零与缺失案例。
- 完整设计、兼容边界和验收证据见 [`DataWork V1.6 更新计划与实施记录`](docs/V1.6_UPDATE_PLAN.md)及[`可复现性记录`](docs/REPRODUCIBILITY.md)。
...
- [V1.6 配对列参数体验与数学绝对值更新计划与实施记录](docs/V1.6_UPDATE_PLAN.md)
- [V1.7 计算核心审计与 MANOVA 因变量组合更新计划与实施记录](docs/V1.7_UPDATE_PLAN.md)
