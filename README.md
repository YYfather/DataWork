...
> 当前可运行版本为 `1.7.0`。V1.7 完成全部计算核心状态审计，并为专业模式 MANOVA 增加因变量无序组合批量计算；V1.5/V1.6 已保存计划继续按原任务与配对公式执行。

## 工作树边界

- `main` 工作树是 1.7.0 完整源码，包含 Python 核心、前端、桌面外壳、测试、审计资料和构建器；不保留服务器部署副本或旧 `发布包/`。
